...
function renderUploadPage({ identity, error = '', success = false, presentation = null }) {
  ...
  if (identity.sessionId) actionParams.set('sessionId', identity.sessionId);
  if (identity.studentId) actionParams.set('studentId', identity.studentId);
  if (identity.studentName) actionParams.set('studentName', identity.studentName);
  if (identity.returnUrl) actionParams.set('returnUrl', identity.returnUrl);

  const action = `/upload-page?${actionParams.toString()}`;
  const returnLink = identity.returnUrl
    ? `<a class="button secondary" href="${escapeHtml(identity.returnUrl)}">Повернутися на сторінку запису</a>`
    : '';
...
<body>
  <main>
    ...
    <p>Це локальна сторінка Electron Agent на ПК захисту. Файл збережеться на цьому ПК і буде синхронізований у Firebase як статус та метадані.</p>
    <table>
    ...
    <p class="hint">Дозволені формати: PDF, PPTX, PPT, ODP. PPTX/PPT/ODP Agent спробує конвертувати у PDF локально, не змінюючи оригінальний файл.</p>
    ${status}
    <form method="post" action="${escapeHtml(action)}" enctype="multipart/form-data">
      ...
      <label><b>Оберіть презентацію</b><input name="presentation" type="file" accept=".pdf,.pptx,.ppt,.odp" required></label>
      <button type="submit">Завантажити в Agent</button>${returnLink}
    </form>
  </main>
</body>
</html>`;
