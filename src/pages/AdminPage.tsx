...
function QueuePanel({ state, setState, session, onEdit }: { state: AppState; setState: (s: AppState) => void; session: DefenseSession; onEdit: (s: Student) => void }) {
  ...
  function markProblem(student: Student) {
    const reason = window.prompt('Опишіть проблему захисту. Цей текст збережеться в примітках студента.')
    if (reason === null) return
    setState(setDefenseStatus(state, student.id, 'problem', reason || 'Проблему не уточнено'))
  }
  return <div>
    <h1>Черга захисту</h1>
    ...
    <div className="panel">
      <h2>Поточна черга</h2>
      <table>
        <thead><tr><th>№</th><th>ПІБ</th><th>Преза</th><th>Захист</th><th>Дії</th></tr></thead>
        <tbody>{queue.map((q) => {
          ...
          return <tr key={q.id}>
            ...
            <td className="actions">
              ...
              <button onClick={() => setState(reorderQueue(state, session.id, s.id, 1))}>↓</button>
              <button onClick={() => setState(requestOpenPresentation(state, session.id, s.id))}>Відкрити презу</button>
              ...
              <button onClick={() => markProblem(s)}>Проблема</button>
