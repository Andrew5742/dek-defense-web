...
import { confirmMobileRegistration, subscribeMobileCompanion, type MobileCompanionSnapshot } from '../services/firebaseAdapter'
...
export function MobileCompanionPage({ token }: Props) {
  ...
  if (!student) return <Screen><div style={styles.error}>Студента не знайдено або посилання застаріло.</div></Screen>
  ...
  return (
    <Screen>
      ...
      {hasProblem ? (
        ...
        <section style={styles.successBox}>Проблеми вирішено, роботу прийнято. Сторінка деактивується через 10 хвилин.</section>
      ) : defendedClean ? (
        <section style={styles.successBox}>Захист завершено успішно. Якщо проблем не виявлено, сторінка деактивується через 10 хвилин.</section>
